...
def render_sidebar(current_module: str):
    with st.sidebar:
        ...
        st.markdown('<div class="sidebar-subtitle">Cultural Risk • ProfitLab • Puente</div>', unsafe_allow_html=True)
        st.markdown("---")
        ...
        module = st.radio(
            "选择平台模块",
            ["平台总览", "文化风控数据库", "ProfitLab 利润量化", "Puente 履约清关"],
            index=["平台总览", "文化风控数据库", "ProfitLab 利润量化", "Puente 履约清关"].index(current_module),
        ...
        st.markdown("---")
        st.caption("平台定位：文化转译 - 利润测算 - 本地交付")
...
def render_overview(account, market):
    ...
    st.markdown('<div class="hero-title">灵径智链 · 拉美国潮出海一体化平台</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-subtitle">围绕“文化转译 - 利润测算 - 本地交付”构建统一工作台</div>', unsafe_allow_html=True)
    ...
    with top_left:
        ...
        st.markdown("""
        当前首页改为 **分区式工作台**，把三大功能拆成独立入口与独立信息区，避免首页信息堆叠。
        你可以按 **文化风控 → ProfitLab → Puente** 的顺序完成从上架预审、利润测算到履约清关的整体验证。
        """)
    ...
    with c1:
        st.markdown(
            '<div class="module-card"><div class="module-title">灵径文化风控数据库</div><div class="module-desc">先做国家文化语义预审，查看颜色禁忌、命名风险、场景误读和营销注意事项。</div><span class="module-chip">国家规则库</span><span class="module-chip">上架预审</span><span class="module-chip">文案替换建议</span></div>',
            unsafe_allow_html=True,
        )
        st.caption('适合先判断“能不能卖、怎么表达更安全”。')
    with c2:
        st.markdown(
            '<div class="module-card"><div class="module-title">ProfitLab 利润量化</div><div class="module-desc">再做成本拆解、BEP 测算、情景模拟和多 SKU 对比，确认定价边界与投放空间。</div><span class="module-chip">BEP 曲线</span><span class="module-chip">情景模拟</span><span class="module-chip">多 SKU 对比</span></div>',
            unsafe_allow_html=True,
        )
        st.caption('适合回答“卖多少钱、多少单回本、怎么比方案”。')
    with c3:
        st.markdown(
            '<div class="module-card"><div class="module-title">Puente 履约清关</div><div class="module-desc">最后做订单追踪、清关预审、报关生成和系统对接，把前面的判断落到履约执行。</div><span class="module-chip">订单追踪</span><span class="module-chip">清关预审</span><span class="module-chip">系统工具</span></div>',
            unsafe_allow_html=True,
        )
        st.caption('适合处理“怎么发、怎么清关、怎么接平台”。')
    ...
    with row1_left:
        st.markdown('<div class="soft-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">文化风控区</div>', unsafe_allow_html=True)
        st.markdown("""
        - 国家风险规则库
        - 商品命名与颜色风险检查
        - 上架前文案替代表达
    ...
    with row1_right:
        st.markdown('<div class="soft-card">', unsafe_allow_html=True)
        st.markdown('<div class="section-title">履约清关区</div>', unsafe_allow_html=True)
        st.markdown("""
        - 订单追踪
        - 清关文件预审
        - 报关生成
        ...
        roadmap = pd.DataFrame(
            {
                ...
                "关键输出": ["规则表、利润模型、演示样板", "数据看板、反馈报告、清关样例", "节气/IP 系列与托管服务包"],
            }
        )
...
def render_cultural_risk(market):
    apply_base_css(False)
    st.markdown('<div class="hero-title">灵径 · 文化风控数据库</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-subtitle">拉美核心市场文化语义、颜色禁忌、营销注意事项与上架前风险预审</div>', unsafe_allow_html=True)
...
def render_profitlab():
    apply_base_css(True)
    st.markdown('<div class="top-brand"><div><div class="hero-title">PROFITLAB</div><div class="hero-subtitle">Profit Analytics · v4 Demo</div></div><div class="status-live">● LIVE &nbsp;&nbsp; %s</div></div>' % datetime.now().strftime("%H:%M:%S"), unsafe_allow_html=True)
    ...
    with right:
        ...
        with tabs[3]:
            ...
            bullets = [
                ...
                f"当前 BEP 为 {metrics['break_even'] or '无法达成'} 件；如果做墨西哥市场节庆活动，建议先用工艺扇/香囊承担引流，不要让高价 SKU 扛第一波转化。",
                "如果你接下来把文化风控模块里的命名与颜色建议也接进来，可以形成“风险预审 -> 定价边界 -> 履约执行”的完整链路。",
            ]
...
def render_puente(account):
    apply_base_css(False)
    st.markdown('<div class="hero-title">Puente Logistics</div>', unsafe_allow_html=True)
    st.markdown('<div class="hero-subtitle">订单追踪 · 清关预审 · 系统工具 · 数据看板</div>', unsafe_allow_html=True)

    tabs = st.tabs(["订单追踪", "数据看板", "清关中心", "系统工具"])
